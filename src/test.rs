    ...
    T: AsRef<str>,
{
    let got: Vec<Vec<&str>> = got
        .iter()
        .map(|row| row.iter().map(|f| f.as_ref()).collect())
        .collect();
...
macro_rules! parses_to {
    ...
    ($name:ident, $csv:expr, $vec:expr, $config:expr) => {
        ...
        fn $name() {
            ...
            rdr = $config(rdr);
            let rows = rdr
                .map(|r| r.and_then(|r| r.decode()).unwrap())
...
parses_to!(
    one_row_trailing_comma_lf,
    "a,b,\n",
    vec![vec!["a", "b", ""]]
);
parses_to!(one_row_one_field_crlf, "a\r\n", vec![vec!["a"]]);
parses_to!(
    one_row_many_fields_crlf,
    "a,b,c\r\n",
    vec![vec!["a", "b", "c"]]
);
parses_to!(
...
parses_to!(empty_string_headers, "", vec![], |rdr: Csv<_>| rdr
    .has_header(true));
parses_to!(
    empty_lines,
    "\n\n\n\n",
...
fail_parses_to!(
    inner_quote_without_quoted_column,
    "a,b,c\"\"d,e",
    vec![vec!["a", "b", "c\"d", "e"]]
);

parses_to!(delimiter_tabs, "a\tb", vec![vec!["a", "b"]], |rdr: Csv<
    _,
>| rdr
    .delimiter(b'\t'));
parses_to!(delimiter_weird, "azb", vec![vec!["a", "b"]], |rdr: Csv<
    _,
>| rdr
    .delimiter(b'z'));

parses_to!(headers_absent, "a\nb", vec![vec!["b"]], |rdr: Csv<_>| rdr
    .has_header(true));
...
fn no_headers_first_record() {
    ...
    assert_eq!(
        r.columns().unwrap().collect::<Vec<_>>(),
...
fn seeking() {
...

    {
        ...
        let vals = d
            .map(|r| r.unwrap().decode::<(usize, usize)>().unwrap())
            .collect::<Vec<_>>();
